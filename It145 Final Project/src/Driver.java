...
public class Driver {
    ...
    public static void main(String[] args) {
        ...
        //Displays menu with options
            ...
            //Starts a loop to continue till x is input
            while ((!userInput.equalsIgnoreCase("x"))) {
                ...
                //displays menu again every time user is returned to menu
                    displayMenu();
                userInput = scn.next();
            }
    ...
    public static void addShip() {
        ...
        //Ensures the ship doesn't already exist
        for (Ship eachShip: shipList) {
            if(eachShip.getShipName().equalsIgnoreCase((newShipName))) {
                System.out.println("The ship is already exists. Exiting to menu...");
                return; //Quits addShip method processing
            }
        }

        //Get number of rooms for  different sections of ship
        System.out.println("Enter number of rooms on the balcony: ");
        int balconyRooms = newShipInput.nextInt();

        System.out.println("Enter number of rooms with oceanview: ");
        ...
        String userInput = newShipInput.nextLine();

        //Adds new ship to list based on user activity
        if(userInput.equalsIgnoreCase("yes")){
            Ship newShip = new Ship(newShipName, balconyRooms, oceanviewRooms, suiteRooms, interiorRooms, true);
            shipList.add(newShip);
        }
        else if(userInput.equalsIgnoreCase("no")){
            Ship newShip = new Ship(newShipName, balconyRooms, oceanviewRooms, suiteRooms, interiorRooms, false);
            shipList.add(newShip);
        }
        else{
    ...
    public static void addCruise() {
        ...
        System.out.println("Enter name of new cruise: ");
        String newCruiseName = newCruiseInput.nextLine();

        //Checks to make sure cruise isn't already on list
        for (Cruise eachCruise: cruiseList){
            if (eachCruise.getCruiseName().equalsIgnoreCase(newCruiseName)){
                System.out.println("That cruise is already exists. Exiting to menu...");
                return;
            }
        }

        //Checks to make sure ship exists in system
        System.out.println("Enter name of ship associated with cruise: ");
        String cruiseShip = newCruiseInput.nextLine();

        //Tests to make sure ship is on list
        for(Ship eachShip: shipList) {
            if (eachShip.getShipName().equalsIgnoreCase(cruiseShip)) {
            } else {
                System.out.print("Ship not in system. Exiting to menu...");
                return;
            }
        }

        //Takes input for different ports for the ship
        System.out.println("Enter name of departure port: ");
        String departureLocation = newCruiseInput.nextLine();

        System.out.println("Enter destination of cruise: ");
        String cruiseDestination = newCruiseInput.nextLine();

        System.out.println("Enter port cruise is returning to: ");
        String returnLocation = newCruiseInput.nextLine();

        //Adds new cruise to list
        Cruise newCruise = new Cruise(newCruiseName, cruiseShip, departureLocation, cruiseDestination, returnLocation);
        cruiseList.add(newCruise);
    }
